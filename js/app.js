...
function renderAlertasDashboard() {
  ...
  STATE.tcs.filter(tc=>tc.activa!==false).forEach(tc=>{
    const pct=pctUsoTC(tc), est=estadoTC(pct), venc=estadoVencimiento(tc);
...
function renderTCs() {
  ...
  el.innerHTML=STATE.tcs.filter(tc=>tc.activa!==false).map(tc=>{
    const cupo=tc.cupo||0,usado=tc.usado||0,libre=Math.max(0,cupo-usado);
    const pct=cupo>0?Math.round((usado/cupo)*100):0;
    const est=estadoTC(pct),venc=estadoVencimiento(tc);
    return `
    <div class="tc-card">
      <div class="tc-header">
...
function renderFondos() {
  ...
  el.innerHTML=STATE.fondos.map(f=>{
    ...
    // Calcular ingresado al fondo desde movimientos
    const ingresado=STATE.movimientos
      .filter(m=>m.tipo===OP.INGRESO&&Array.isArray(m.distribucion))
      .reduce((s,m)=>{
        const d=m.distribucion.find(d=>d.fondoId===f.id);
        return s+(d?.monto||0);
      },0);
    const gastado=Math.max(0,ingresado-saldo);
    const pctGasto=ingresado>0?Math.min(100,Math.round((gastado/ingresado)*100)):0;

    // Montos por ítem calculados como % del ingresado
    const itemsHtml=mostrarDetalleItems&&(f.items||[]).length>0?`
      <div style="margin-top:8px;padding-top:8px;border-top:1px solid var(--border)">
        ${f.items.map(it=>{
          const montoItem=ingresado>0?Math.round(ingresado*(it.pctDelFondo||0)/100):0;
          return `<div style="display:flex;justify-content:space-between;font-size:12px;color:var(--text3);padding:3px 0">
            <span>${it.nombre}</span>
            <span style="display:flex;gap:10px">
              <span>${it.pctDelFondo||0}%</span>
              ${ingresado>0?`<span style="color:var(--text2)">${formatCLP(montoItem)}</span>`:""}
              ${it.presupuestado?`<span style="color:var(--accent)">${formatCLP(it.montoPresupuesto||0)} pres.</span>`:""}
            </span>
          </div>`;
        }).join("")}
      </div>`:""
      ...
      ${ingresado>0?`
        <div style="margin-top:6px">
          <div style="display:flex;justify-content:space-between;font-size:11px;color:var(--text3);margin-bottom:3px">
            <span>Ingresado: ${formatCLP(ingresado)}</span>
            <span>Gastado: ${formatCLP(gastado)} (${pctGasto}%)</span>
          </div>
          ...
          <div style="height:2px;background:rgba(255,255,255,0.1);border-radius:99px"></div>
        </div>`:""}
  ...
  const h=Math.max(300,STATE.fondos.length*65);
...
function previsualizarDistribucion() {
  ...
  if(!dist.length){el.innerHTML=`<p style="font-size:13px;color:var(--text3)">Sin reglas — ve a Ajustes → Ingresos</p>`;return;}
...
function renderDeudaManual() {
  const el=document.getElementById("lista-deuda");
  el.innerHTML=STATE.tcs.filter(tc=>tc.activa!==false&&(tc.usado||0)>0).map(tc=>`
    <div class="deuda-item">
      <div class="di-header">
...
async function init() {
  ...
  if(!STATE._online){
    ...
    if(!ok) mostrarError("Sin conexión y sin datos locales.");
  } else {
